...
The v2 pipeline (spec: the design spec). The
...
> **Update:** the extension now captures application questions via the
> deterministic ON-PAGE FIELD PICKER (`extension/ui/field-picker.js` +
> `lib/question-mapper.js`; spec the design spec)
...
Every paid seam (AI, Firecrawl, R2, Resend, QStash, Neon) is guarded by per-user / per-IP rate
limiting — the first step inside a handler, right after `getUserId(req)` (`lib/server/ratelimit.ts`,
Upstash Redis, fail-open). Blocks return **429 + `Retry-After`** with system-framed copy. Ceilings
sit far above real use, so honest users never notice; document storage also has a per-user stock cap
in `createDocument`. Full policy table + provider spend-cap checklist in
[`RATE_LIMITING.md`](RATE_LIMITING.md). When adding a new paid route, add the matching `limit*` call.
